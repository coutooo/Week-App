...
class OutfitWidgetVisiting extends StatefulWidget {
  final UserModel user;
  const OutfitWidgetVisiting({required this.user, Key? key}) : super(key: key);

  @override
  State<OutfitWidgetVisiting> createState() => _OutfitWidgetVisitingState();
}

class _OutfitWidgetVisitingState extends State<OutfitWidgetVisiting> {
  ...
  Future getPhotos(DateTime dateTime, int duration) async {
    ...
    var res = await dbHelper.photoToday(widget.user.user_id, currDate,
  ...
  void getList() async {
    if (pid != null) {
      print('getting list');
      var res = await dbHelper.getClothing(widget.user.user_id, pid);
      if (res != null) {
